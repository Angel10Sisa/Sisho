...
class OptionService implements Crud<Option> {
   ...
   */
  async create(option: Partial<Option>): Promise<Option> {
  ...
  async find(filter?: Filter<Option>): Promise<Option[]> {
    const res = await get('/api/options', { filter })
  ...
  async findById(id: number, filter?: Filter<Option>): Promise<Option> {
    const res = await get({ url: '/api/option/{id}', params: { id } }, { filter })
    const data: Option = await res.json()
    return data
  }
   ...
   */
  async updateById(id: number, option: Partial<Option>): Promise<void> {
